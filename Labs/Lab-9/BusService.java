...
class BusService {
     ...
     * @return A future set of bus stops that this bus services serves.
     */
    public CompletableFuture<Set<BusStop>> getBusStops() {
        CompletableFuture<Scanner> scanner = BusAPI.getBusStopsServedBy(serviceId)
            .thenApply((x) -> new Scanner(x));
        CompletableFuture<Set<BusStop>> stops = scanner
            .thenApply((sc) -> {
    ...
    /**
     * Return a future set of bus stops matching a given name.
     * @param  name Name (possibly partial) of a bus stop.
     * @return A future set of bus stops matching the given name.
     */
