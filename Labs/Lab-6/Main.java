...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int index = 0; index < students.size(); index += STUDENT_CLASS_LENGTH) {
            ...
            Assessment addAssessment = new Assessment(assessement, grade);
            Module addModule = new Module(module).put(addAssessment);
            Runnable insertModuleIntoStudent = () -> roster.get(name)
                .map((x) -> x.put(addModule))
                .orElseThrow(); // will never face this error lol
            Runnable insertStudentIntoRoster = () -> roster.put(stressedOutStudent);


            roster.get(name)
                .ifPresentOrElse((x) -> x.get(module)
                .ifPresentOrElse((y) -> y.put(addAssessment), 
                    insertModuleIntoStudent), 
                        insertStudentIntoRoster);
        }
